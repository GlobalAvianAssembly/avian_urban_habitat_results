...
library(grid)
library(gridExtra)
birdlife_plot_ldm = ggplot(birdlife_data_with_lat, aes(x = city_centre_latitude, y = response)) + 
  geom_point(size=1, alpha = 0.2) + 
  geom_smooth(method = "glm", formula = y ~ I(x^2)) +
  geom_text_repel(aes(label = name), data = outside_se, size = 3, nudge_y = 1, color = "red") +
  geom_point(data = outside_se, aes(color = region_50km_ssm)) +
  scale_colour_gradientn(colours = c("yellow", "red")) +
  guides(colour=guide_legend(title="SSM in 50km Surrounding city")) +
  theme_bw() +
  ylab("City Random Effect Intercept") + xlab("Latitude (of city centre)") + labs(title = "Birdlife") +
  theme(legend.position="bottom", legend.box = "horizontal")

grid.arrange(birdlife_plot_ldm + theme(legend.position = 'none'))
ggsave('city_effect_richness__output__ldg_birdlife_with_ssm.jpg')
```
...
birdlife_ldm_legend <- g_legend(birdlife_plot_ldm)
grid.newpage()
grid.draw(birdlife_ldm_legend) 

jpeg('city_effect_richness__output__ldg_birdlife_with_ssm__legend.jpg', width = 500, height = 100)
grid.newpage()
grid.draw(birdlife_ldm_legend) 
dev.off()
```
