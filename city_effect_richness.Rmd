...
-----------------------------------------------
-- best model with merlin is pool size + realm
-----------------------------------------------
...
```
```{r}
city_data[c(16, 53, 124), c("name", "birdlife_pool_size", "merlin_pool_size")]
```
...
ggplot(birdlife_city_data_fixed_no_boreal, aes(x = birdlife_pool_size, y = response)) + 
  ...
  geom_text(aes(label = name), data = birdlife_city_data_fixed_no_boreal[c(16, 53, 126),], size = 4, position = "dodge", vjust = "inward", hjust = "inward") +
  facet_wrap(~ realm) +
  theme_bw()
```
