...
merlin_richness_plot$type[merlin_richness_plot$explanatory %in% c('mean_elevation_scaled', 'elevation_delta_scaled')] <- 'Average elevation\n5 km around site'
...
ggplot(merlin_richness_plot, aes(y=explanatory, x=richness_merlin_estimate, colour = type)) + 
  ...
  guides(colour=guide_legend(title="Explanatory type")) + xlab('Increase in percentage of regional richness\n± Standard Error') + ylab('Explanatory')
...
birdlife_richness_plot$type[birdlife_richness_plot$explanatory %in% c('mean_elevation_scaled', 'elevation_delta_scaled')] <- 'Average elevation\n5 km around site'
...
ggplot(birdlife_richness_plot, aes(y=explanatory, x=richness_birdlife_estimate, colour = type)) + 
  ...
  theme_bw() +
  geom_vline(xintercept=0, linetype="dotted") +
  guides(colour=guide_legend(title="Explanatory type")) + xlab('Increase in percentage of regional pool richness\n± Standard Error') + ylab('Explanatory')
...
```{r}
both_niche_resiliance_city_result <- model_average_richness(both_city_response)
...
```{r}
either_niche_resiliance_city_result <- model_average_richness(either_city_response)
