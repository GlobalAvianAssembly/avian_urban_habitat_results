...
model_average_for_city <- function(data) {
  model_dredge(
    lm(response ~ population_scaled + area_scaled + urban + biome_name + city_includes_estuary + region_includes_estuary + city_mean_elevation_scaled + city_elevation_delta_scaled + region_mean_elevation_scaled + region_elevation_delta_scaled + population_growth_scaled, data), data, 4)
}
```
...
```
```{r}
write_csv(merlin_city_response[, c('name', 'response', 'latitude', 'longitude')], 'merlin_city_richness_intercept.csv')
```
...
birdlife_city_response[order(birdlife_city_response$response), c('name', 'response', 'latitude', 'longitude')]
```

```{r}
write_csv(birdlife_city_response[, c('name', 'response', 'latitude', 'longitude')], 'birdlife_city_richness_intercept.csv')
```
...
both_city_response[order(both_city_response$response), c('name', 'response', 'latitude', 'longitude')]
```
...
either_city_response[order(either_city_response$response), c('name', 'response', 'latitude', 'longitude')]
```
