...
```{r}
model_average_richness_with_num_checklists <- function(data) {
  data$number_of_checklists_scaled = rescale(data$number_of_checklists, to = c(0, 1), from = range(data$number_of_checklists, na.rm = TRUE, finite = TRUE))
...
-----------------------
Birdlife with number of checklists as covariate
----------------------
```{r}
bl_with_num_cl_richness_result = model_average_richness_with_num_checklists(birdlife)
bl_with_num_cl_richness_result
```


```{r}
bl_with_num_cl_richness_sum <- model_summary('birdlife_with_num_cl', 'richness', bl_with_num_cl_richness_result)
bl_with_num_cl_richness_sum
```


```{r}
bl_with_num_cl_richness_plot <- bl_with_num_cl_richness_sum[bl_with_num_cl_richness_sum$model == 'full',]
bl_with_num_cl_richness_plot$explanatory <- factor(bl_with_num_cl_richness_plot$explanatory, levels = c('closed_forest', 'open_forest', 'shrubs', 'herbaceous_vegetation', 'herbaceous_wetland', 'permanent_water', 'cultivated', 'urban', 'mean_elevation_scaled', 'elevation_delta_scaled', 'realmAfrotropic', 'realmAustralasia', 'realmIndomalayan', 'realmNearctic', 'realmNeotropic', 'number_of_checklists_scaled'))
bl_with_num_cl_richness_plot$type <- 'Proportion of landcover\n5 km around site'
bl_with_num_cl_richness_plot$type[bl_with_num_cl_richness_plot$explanatory %in% c('mean_elevation_scaled', 'elevation_delta_scaled')] <- 'Scaled (0-1) average\nelevation 5 km around site'
bl_with_num_cl_richness_plot$type[bl_with_num_cl_richness_plot$explanatory %in% c('realmAfrotropic', 'realmAustralasia', 'realmIndomalayan', 'realmNearctic', 'realmNeotropic')] <- 'Realm'
bl_with_num_cl_richness_plot$type[bl_with_num_cl_richness_plot$explanatory %in% c('number_of_checklists_scaled')] <- 'Scaled (0 - 1) number\nof checklists'

bl_with_num_cl_plot = ggplot(bl_with_num_cl_richness_plot, aes(y=explanatory, x=richness_birdlife_with_num_cl_estimate, colour = type)) + 
  geom_line() +
  geom_point()+
  geom_errorbar(aes(xmin=richness_birdlife_with_num_cl_estimate-richness_birdlife_with_num_cl_error, xmax=richness_birdlife_with_num_cl_estimate+richness_birdlife_with_num_cl_error), width=.2,
                 position=position_dodge(0.05)) +
  scale_y_discrete(
    limits = rev(levels(bl_with_num_cl_richness_plot$explanatory)), 
    labels = c('closed_forest' = 'Closed forest', 'open_forest' = 'Open forest', 'shrubs' = 'Shrubs', 'herbaceous_vegetation' = 'Herbaceous\nvegetation', 'herbaceous_wetland' = 'Herbaceous\nwetland', 'permanent_water' = 'Permanent\nwater', 'cultivated' = 'Cultivated', 'urban' = 'Urban', 'mean_elevation_scaled' = 'Mean elevation\nscaled', 'elevation_delta_scaled' = 'Elevation delta\nscaled', 'realmAfrotropic' = 'Afrotropical', 'realmAustralasia' = 'Austaliasian', 'realmIndomalayan' = 'Indomalayan', 'realmNearctic' = 'Nearctic', 'realmNeotropic' = 'Neotropical', 'number_of_checklists_scaled' = 'Number\nof checklists')) +
  theme_bw() +
  geom_vline(xintercept=0, linetype="dotted") +
  guides(colour=guide_legend(title="Predictor type")) + xlab('Increase in proportion of regional pool richness\n± Standard Error') + ylab('Predictor') +
  theme(legend.justification = "top")
bl_with_num_cl_plot
ggsave('percentage_of_regional_pool__output__birdlife_with_num_cl_richness_result.jpg')
```
